		...
		if (Number(req.headers.api_version) === 2) {
			...
			switch (params.task_name) {
						...
						var output2 = await cq(
							...
							`insert into users (username,hashed_password,email,phone_number,time) values ('${
								...
								!params.email_address
									? "NULL"
									: JSON.stringify(params.email_address)
							},${
								...
								if (results.length === 0) {
									...
									rm.send_result(
										results[0].hashed_password ==
											hash_sha_256_hex(
												params.password +
													process.env.PASSWORD_HASHING_SECRET
											)
									);
								}
							}
				...
				case "set_company_info":
					...
					var o = await cq(
						con,
						'delete from paired_data where pair_key = "company_info"'
					);
					if (o.error) {
						rm.send_error(o.error);
						break;
				...
				case "delete_user":
					var r = await cq(
						con,
						`delete from users where username = '${params.username}'`
					);
					if (r.error) {
				...
				*/
				...
				case "delete_icon":
					...
					var icon_file_name = file_names.find(
						(i) => i.split(".")[0] === params.icon_type
					);
					if (icon_file_name) {
						fs.rmSync(path.join("./uploaded/company_info", icon_file_name), {
							force: true,
				...
				case "new_download_center_item":
					var current_download_center_items = fs.readdirSync(
						"./uploaded/download_center"
					);
					var file_name = params.title;

					if (
						current_download_center_items
							.map((i) => i.split(".")[0])
							.includes(file_name)
					) {
						rm.send_error(
				...
				case "remove_download_center_item":
					o = await cq(
						con,
						`delete from download_center where file_name="${params.title}"`
					);
					if (o.error) {
						rm.send_error(o.error);
						break;
